...
// Note: I'm using this package here to get the underlying rep
...
func (q *Queue) resize(size int) {
	adjusted := make([]*AbstractAction, size)
	if q.front < q.back {
...
func (q *Queue) PushBack(v *AbstractAction) {
	q.lazyInit()
	q.lazyGrow()
	q.rep[q.back] = v
	q.back = q.inc(q.back)
...
func (q *Queue) PopFront() *AbstractAction {
	if q.empty() {
		return nil
	}
	v := q.rep[q.front]
...
func (q *Queue) PopBack() *AbstractAction {
	if q.empty() {
		return nil
	}
	q.back = q.dec(q.back)
