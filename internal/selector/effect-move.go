...
func (e *MoveEffect) Draw(target pixel.Target) {
	if len(e.area) > 0 && e.area[len(e.area)-1] != e.orig {
		if e.End {
			...
			SelectionSprites["move_u_d"].Draw(target, e.matrices[0][1])
			SelectionSprites["move_start_u"].Draw(target, e.matrices[0][0])
			SelectionSprites["move_end_alone"].Draw(target, e.matrices[0][2])
		} else {
